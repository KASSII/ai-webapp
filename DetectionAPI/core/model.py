...
# 10層にわたる、extrasモジュールを作成(SSD512用)
def make_extras512():
    layers = []
    in_channels = 1024  # vggモジュールから出力された、extraに入力される画像チャネル数
    ...
    layers += [nn.Conv2d(cfg[5], cfg[6], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[6], cfg[7], kernel_size=(3), stride=2, padding=1)]
    layers += [nn.Conv2d(cfg[5], cfg[6], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[6], cfg[7], kernel_size=(4), padding=1)]
    return nn.ModuleList(layers)
...
def make_loc_conf(num_classes=21, bbox_aspect_num=[4, 6, 6, 6, 4, 4]):
    ...
    # (補足)
    # kernel_size=3, padding=1, stride=1(default)にすると出力のサイズは入力サイズと同じになる
    # したがって、loc_layerの出力は(38, 38, 16)になる。
    # ロスを計算するときは、この出力を(height*width*aspect_num, 4)、つまり(38*38*4, 4)にreshapeして各デフォルトボックスに対するオフセットに変換する
    ...
    for n in bbox_aspect_num[3:]:
        loc_layers += [nn.Conv2d(256, n
                                 * 4, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(256, n
                                  * num_classes, kernel_size=3, padding=1)]
...
class DBox(object):
    def __init__(self, cfg):
        ...
        # 初期設定
        self.image_size = cfg['input_size']  # 画像サイズ
        self.feature_maps = cfg['feature_maps']  # 各sourceの特徴量マップのサイズ（SSD300の場合 [38, 19, …]）
        self.num_priors = len(cfg["feature_maps"])  # sourceの個数（SSD300の場合 6）
        self.steps = cfg['steps']  # [8, 16, …] DBoxのピクセルサイズ（SSD300の場合 6）
        self.min_sizes = cfg['min_sizes']  # 小さい正方形のDBoxのピクセルサイズ（SSD300の場合 [30, 60, …]）
        self.max_sizes = cfg['max_sizes']  # 大きい正方形のDBoxのピクセルサイズ（SSD300の場合 [60, 111, …]）
                                           # (補足) 正確には、1辺の長さが sqrt(min_size*max_size)[pix] の矩形となる
...
class Detect(Function):
    ...
    def forward(ctx, loc_data, conf_data, dbox_list, conf_thresh=0.01, top_k=200, nms_thresh=0.45):
        ...
        dbox_list: [8732,4]
            DBoxの情報
        conf_thresh: float
            出力対象のbboxのconfのしきい値（confがconf_threshより高いDBoxのみを扱う）
        top_k: int
            nm_supressionで使用するbboxの数（confの高いtop_k個を計算に使用する）
        nms_thresh: float
            nm_supressionで同一物体とみなすIoUのしきい値（IOUがnms_threshより大きいと、同一物体へのBBoxとみなす）
        ...
        # dbox_listをloc_dataと同じデバイスに転送
        dbox_list = dbox_list.to(loc_data.device)
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, cfg):
        ...
        # SSDのネットワークを作る
        self.vgg = make_vgg()
        if cfg["input_size"] == 300:
            self.extras = make_extras300()
        elif cfg["input_size"] == 512:
            self.extras = make_extras512()
        self.L2Norm = L2Norm()
    ...
    def forward(self, x):
        ...
        # confのサイズはtorch.Size([batch_num, 183372])になる(SSD300の場合)
